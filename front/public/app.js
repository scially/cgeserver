...
var autoConnect
var receivedBytesMeasurement //未全局定义打包报警告
var receivedBytes //未全局定义打包报警告
...
function showConnectOverlay(wsUrl) {
...
function setupWebRtcPlayer(htmlElement, config) {
  ...
  // webRtcPlayerObj.video.muted = "muted";
  // webRtcPlayerObj.video.autoplay = true;
...
function registerKeyboardEvents() {
  ...
  document.onkeydown = function (e) {
    ...
    // cancle f1-f12
    if (e.keyCode >= SpecialKeyCodes.F1 && e.keyCode <= SpecialKeyCodes.F12) return;
...
function connect(wsUrl) {
  ...
  // console.log('ws://' + window.link.hostname + ':' + window.link.playerWs);
  // ws = new WebSocket('ws://' + window.link.hostname + ':' + window.link.playerWs);
  ...
  ws.onclose = function (event) {
    ...
    var reclickToStart = setTimeout(start(wsUrl), 4000);
  };
